    ...
    def _parse_element(self, inp):
        try:
            ...
            raise DTDParsingError(
                "Incomplete element definition in line " + str(self._line)
            )
        ...
        if data.startswith("("):
            if data.endswith(")"):
                ...
                raise DTDParsingError(
                    "Element definition error: unbalanced parentheses"
                    " in line " + str(self._line)
                )
        while data:
            if data.startswith("("):
                test = data.split(")", 1)
...
def main():
    ...
    for i, item in enumerate(data):
        print(("--- item", i, ":"))
        for key, val in list(item.__dict__.items()):
            print((key, val))
        # try:
        for sub in item.subelement_list:
            print("------subelem:")
            for key, val in list(sub.__dict__.items()):
                print((key, val))
