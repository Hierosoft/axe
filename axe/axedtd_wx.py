...
def getshortname(x, attr=False, ent=False):
    if attr:
        name, srt, opt, val = x
        strt = " ".join(
            (SYMBOLS["attsrt"][srt][0], name, SYMBOLS["attwrd"][opt][0], val)
        )
        t = "".join(("[", "]"))
    elif ent:
        name, srt, val = x
...
def is_element(data):
    test = data.split()[0]
    if test in [x[0] for x in list(SYMBOLS["elsrt"].values())]:
        return True
    else:
        return False
...
def is_attribute(data):
    test = data.split()[0]
    if test in [x[0] for x in list(SYMBOLS["attsrt"].values())]:
        return True
    else:
        return False
...
def is_entitydef(data):
    test = data.split()[0]
    if test in [x[0] for x in list(SYMBOLS["entsrt"].values())]:
        return True
    else:
        return False
...
class ElementDialog(wx.Dialog):
    def __init__(
        ...
        wx.Dialog.__init__(
            self, parent, -1, title=title, size=size
        )  # , pos, size, style)
        self._parent = parent
        ...
        if not_root:
            ...
            self.rbTypes = [
                wx.RadioButton(self.pnl, -1, label=SYMBOLS["elsrt"][name][1])
                for name in ELTYPES
            ]
    ...
    def on_ok(self, ev):
        ...
        if self.not_root and self.rbTypes[0].Value:
            if tag:
                ...
                wx.MessageBox(
                    "Element name must be empty for PCDATA",
                    self._parent.title,
                    wx.OK | wx.ICON_ERROR,
                )
                return
...
class AttributeDialog(wx.Dialog):
    def __init__(
        ...
        wx.Dialog.__init__(
            self, parent, -1, title=title, size=(320, 225)
        )  # ,pos.size,style)
        self._parent = parent
        self.pnl = wx.Panel(self, -1)
        ...
        self.rbWrds = [
            wx.RadioButton(self.pnl, -1, label=SYMBOLS["attwrd"][name][1])
    ...
    def on_ok(self, ev):
        ...
        if nam == "" or len(nam.split()) > 1:
            ...
            wx.MessageBox(
                "Attribute name cannot be empty or contain spaces",
                self._parent.title,
                wx.OK | wx.ICON_ERROR,
            )
            return
        ...
        for key, wrd in list(SYMBOLS["attsrt"].items()):
            if typ == wrd[1]:
...
class EntityDialog(wx.Dialog):
    def __init__(
        ...
        wx.Dialog.__init__(
            self, parent, -1, title=title, size=(320, 160)
        )  # ,pos.size,style)
        self._parent = parent
        self.pnl = wx.Panel(self, -1)
    ...
    def on_ok(self, ev):
        ...
        if nam == "" or len(nam.split()) > 1:
            self.txtName.SetFocus()
            wx.MessageBox(
                "Entity name cannot be empty or contain spaces",
                self._parent.title,
                wx.OK | wx.ICON_ERROR,
            )
            return
...
class MainFrame(wx.Frame):
    def __init__(self, parent, id, fn=""):
        ...
        self.xmlfn = fn
        wx.Frame.__init__(self, parent, id, pos=(2, 2), size=(320, 450))
    ...
    def edit(self, ev=None):
        ...
        if is_element(test):
            ...
            if tag == self.rt.tag:
                edt = ElementDialog(
                    self, title="Edit root element", item=data, not_root=False
                )
            else:
    ...
    def paste(self, ev=None, before=True, pastebelow=False):
        ...
        if data == self.rt:
            if before:
                ...
                wx.MessageBox(
                    "Pasting as first element under root",
                    self.title,
                    wx.OK | wx.ICON_INFORMATION,
                )
                pastebelow = True
    ...
    def insert(self, ev=None, before=True, below=False):
        ...
        if below:
            ...
            if is_pcdata(text):
                ...
                return
        if (
            self.tree.GetItemPyData(self.item) == (self.rt.tag, "one", False)
            and not below
        ):
            wx.MessageBox(
...
def test_is_element():
    for test in [x[0] for x in list(SYMBOLS["elsrt"].values())]:
        assert is_element(" ".join((test, "testdata"))), " ".join(
            (test, "wordt niet herkend als element")
        )
    for test in ("<15> hallo", "", "xxxxx", "hallo daar vrienden"):
        assert not is_element(" ".join((test, "testdata"))), " ".join(
            (test, "wordt herkend als element")
        )


def test_is_attribute():
    for test in [x[0] for x in list(SYMBOLS["attsrt"].values())]:
        assert is_attribute(" ".join((test, "testdata"))), " ".join(
            (test, "wordt niet herkend als attribuut")
        )
    for test in ("<15> hallo", "", "xxxxx", "hallo daar vrienden"):
        assert not is_attribute(" ".join((test, "testdata"))), " ".join(
            (test, "wordt herkend als attribuut")
        )


def test_is_entitydef():
    for test in [x[0] for x in list(SYMBOLS["entsrt"].values())]:
        assert is_entitydef(" ".join((test, "testdata"))), " ".join(
            (test, "wordt niet herkend als entiteit")
        )
    for test in ("<15> hallo", "", "xxxxx", "hallo daar vrienden"):
        assert not is_entitydef(" ".join((test, "testdata"))), " ".join(
            (test, "wordt herkend als entiteit")
        )
